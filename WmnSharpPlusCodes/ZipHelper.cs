...
namespace WmnSharpPlusCodes
{
    public class ZipHelper
    {
        ...
        private void GetAllDirsFiles(IEnumerable<DirectoryInfo> dirs, Hashtable filesList)
        {
        ...
        public bool ZipManyFilesOrDictorys(IEnumerable<string> folderOrFileList, string zipedFile, string password)
        {
            ...
            using (var s = new ZipOutputStream(File.Create(zipedFile)))
            {
                ...
                foreach (string fileOrDir in folderOrFileList)
                {
                    ...
                    {
                        res = ZipFileDirctory(fileOrDir, s, "");
                    }
                    else
                    {
                        //文件
        ...
        private bool ZipFileDirctory(string folderToZip, ZipOutputStream s, string parentFolderName)
        {
            bool res = true;
            ZipEntry entry = null;
            FileStream fs = null;
            ...
            {
                if (!ZipFileDirctory(folder, s, Path.Combine(parentFolderName, Path.GetFileName(folderToZip))))
                {
                    return false;
                }
            }
