...
namespace WmnSharpStdCodes.Text
{
...
, { -10256, "zun" }, { -10254, "zuo" } };
        ...
        {
            var arr = System.Text.Encoding.Default.GetBytes(ch.ToString());//根据系统默认编码得到字节码 
            if (arr.Length == 1)//如果只有1字节说明该字符不是汉字，结束本次循环 
            {
                return false;
            }
            var arr1 = (short)(arr[0]);   //取字节1 
            var arr2 = (short)(arr[1]);   //取字节2 
            var charCode = arr1 * 256 + arr2 - 65536;//计算汉字的编码 
        ...
        /// <returns></returns>
        public static string PinYinFirstChars(string ChineseStr)
        {
            string buf = "";
            foreach (char ch in ChineseStr)
            {
                string pinyin = ToPinYin(ch.ToString());
                if (pinyin.Length > 0)
                {
                    buf += (pinyin.ToCharArray()[0]);
                }
            }
            return buf;
        }
       ...
       /// <returns></returns>
        ...
        {
           var arr = System.Text.Encoding.Default.GetBytes(str);
           return arr.Length;
        }
